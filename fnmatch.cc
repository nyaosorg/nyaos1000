 ...
 *
 * 葉山が中川師のパッチをいただいて、こしらえたバージョン。
 * 関数名が _fnmatch → dbcs_fnmatch になっている他、
 * DBCS 判定関数などを NYAOS 専用のものに差し変えています。
 * なお、バグってたら、葉山の責任なので、葉山以外のところへ
 * 連絡するのはご遠慮ください。
 */
...
#include "macros.h"
#define _nls_is_dbcs_lead(c)	is_kanji(c)
#define _nls_tolower(c)		to_lower(c)
#define _nls_toupper(c)		to_upper(c)
...
static const unsigned char *skip_comp_os2 (const unsigned char *src)
{
  ...
  while (!IS_OS2_COMP_END (*src))
    if (_nls_is_dbcs_lead (*src) && src[1] != 0)
      src += 2;
    else
      ++src;
...
static int has_colon (const unsigned char *p)
{
  while (*p != 0)
    if (*p == ':')
      return 1;
    else if (_nls_is_dbcs_lead (p[0]) && p[1] != 0)
      p += 2;
    else
      ++p;
  return 0;
                           ...
                           unsigned flags, int has_dot)
{
  ...
  for (;;)
    if (_nls_is_dbcs_lead (*mask) && mask[1] != 0)
      {
        /* DBCS characters match themselves. */

        if (mask[0] != name[0] || mask[1] != name[1])
          return FNM_NOMATCH;
        mask += 2; name += 2;
      }
    else
      switch (*mask)
        ...
        case '?':
          ...
          if (*name != '.' && !IS_OS2_COMP_END (*name))
            {
              if (_nls_is_dbcs_lead (name[0]) && name[1] != 0)
                name += 2;
              else
                ++name;
            }
  ...
  switch (flags & _FNM_STYLE_MASK)
    ...
    case _FNM_DOS:
      ...
      while (!IS_OS2_COMP_END (*s) && *s != '.')
        if (_nls_is_dbcs_lead (s[0]) && s[1] != 0)
          s += 2;
        else
          ++s;
...
{
  unsigned char c1, c2;
  unsigned short dbc1, dbc2;	/* Double Byte Character */
  char invert, matched;
  const unsigned char *start;
  int rc;

  for (;;)
    if (_nls_is_dbcs_lead (*mask))
      {
        /* DBCS characters match themselves. */
      ...
      switch (*mask)
...
#if 0
          else if (_nls_is_dbcs_lead (*name))
            {
		...
		if (*mask)
		  ...
		  return _FNM_ERR; /* Illegal DBCS character */
              else
		dbc1 = 0;
		...
		if (dbc2)
		  {
		    if ((_nls_is_dbcs_lead (name[0]) && name[1])
			&& ((dbc1 <= (name[0] * 256 + name[1]))
			    && ((name[0] * 256 + name[1]) <= dbc2)))
		      matched = 1;
		  }
		else
		  return _FNM_ERR; /* all DBCS chars are larger than any SBCS char */
	      else
		if (dbc2)
		  {
		    if (_nls_is_dbcs_lead (name[0]))
		      {
			if (name[1])
			  {
			    if ((name[0] * 256 + name[1]) <= dbc2)
			      matched = 1;
			  }
			else
			  return _FNM_ERR; /* Illegal DBCS character */
...
{
  return _fnmatch_unsigned (  (const unsigned char *)mask
			    , (const unsigned char *)name
			    , flags);
}
